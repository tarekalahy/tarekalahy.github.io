---
layout: post
title:  "Readium LSD Specifications"
...
This specification uses the following EPUB and Readium-LCP terms:
...
In Readium-LCP, the authority that grants the License Document and the Publication to the User, usually a Web Server.
...
<em> In the following example, the Status Document indicates that the License has been activated on a single device and can be updated if wanted.</em>
...
It <b>MUST</b> meet the conformance constraints for JSON documents as defined in [JSON].
...
To provide a consistent behavior for both clients and end users, all servers must handle errors using the Problem Details JSON object as defined in [[RFC7807](https://tools.ietf.org/html/rfc7807)] with the following additional requirements:
...
Each interaction in this specification provides a number of Problem Types that are returned to the client if the interaction fails.
...
During the registration, the client <b>MUST</b> always send the same unique identifier for a specific device, no matter which Status Document it interacts with. Any further interaction **SHOULD** use the same identifier/name.
...
If returning is unsuccessful, the client **SHOULD** attempt to return the License again at a later time without necessarily asking the user again.
...
This specification defines a way for a Content Provider to connect a Readium LCP License Document to a Status Document describing the current status of the underlying license.

It does not add any structural elements to the LCP License Document, but the link relations it defines and the potential rights it enumerates <b>MAY</b> appear in a Readium LCP License Document.
...
If a link with the relation status is present in the `links` object, the client <b>MUST</b> process the Status Document according to the extended rules listed in [section 3](#heading=h.7agurmogq68s) of this specification.
